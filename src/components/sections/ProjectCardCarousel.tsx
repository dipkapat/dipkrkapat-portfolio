...
export function ProjectCardCarousel({ project }: ProjectCardCarouselProps) {
	...
	return (
		...
		>
			...
			>
				{/* Inner Core */}
				<div className="rounded-[20px] bg-bg-0 h-full flex flex-col p-2">
					{/* Image -fixed 4:3 aspect ratio with double-bezel */}
					<a
						href={`/work/${project.slug}`}
						className="group relative block overflow-hidden rounded-2xl border border-bg-3 transition-all duration-300 ease-out hover:border-accent-0 hover:shadow-glow mb-10 flex-shrink-0"
						aria-label={`${project.name} case study`}
					>
						{/* Outer shell */}
						<div className="rounded-2xl border border-bg-3 p-1 transition-all duration-300 ease-out group-hover:border-accent-0">
					...
					<div className="p-6 lg:p-8 flex flex-col flex-1">
						<div className="flex-1">
							...
							<CategoryBadge
								...
								className="mt-2 group-hover:scale-[1.05] transition-transform duration-150 ease-spring"
							/>
