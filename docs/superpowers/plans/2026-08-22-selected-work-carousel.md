			...
			>
				<Container>
					...
					<header className="mb-12 lg:mb-16">
						<h2
							id="work-heading"
							className="font-ui font-semibold text-5xl lg:text-6xl xl:text-7xl tracking-tight text-fg-0"
						>
							Selected Work
						</h2>
						<p className="mt-4 text-lg lg:text-xl text-fg-1 max-w-[60ch]">
							Five projects spanning brand, fintech, SaaS, and AI
							-each designed around real problems and shipped to
							production.
						</p>
					</header>
					...
					{showScrollHint && (
						<motion.div
							...
							tabIndex={0}
							onKeyDown={(e) =>
								e.key === "Enter" && dismissHint()
							}
							aria-label="Dismiss scroll hint"
						>
							<span className="font-mono text-xs uppercase tracking-wider text-fg-2">
								Scroll
							</span>
							<motion.svg
								width="20"
